...
int vs10xx_device_w_WRAM(int id, unsigned char* add, unsigned char* data)
{
	int status = 0;
	
	status = vs10xx_device_w_sci_reg(id, 0x07/*SCI_WRAMADDR*/, add[1], add[0]);
	status = vs10xx_device_w_sci_reg(id, 0x06/*SCI_WRAM*/, data[1], data[0]);
...
static int vs10xx_device_swreset(int id, int testmode) {
	...
	if (testmode) {
		...
		printk("Read SCI_MODE Reg(MSB | LSB): (0x%x | 0x%x)\n", msb, lsb);
	}
	...
	if (status == 0 && clockf != 0) {
		...
		printk("Read SCI_CLOCKF Reg(MSB | LSB): (0x%x | 0x%x)\n", msb, lsb);
	}
...
int vs10xx_device_reset(int id) {
	...
	if (status == 0) {
		...
		printk("Read SCI_STATUS Reg(MSB | LSB): (0x%x | 0x%x)\n", msb, lsb);
	...
	if (status == 0) {
		...
		switch (vs10xx_device[id].version) {
			case 0:
				vs10xx_inf("id:%d found vs1001 device", id);
				break;
			case 1:
				vs10xx_inf("id:%d found vs1011 device", id);
				break;
			case 2:
				vs10xx_inf("id:%d found vs1002 device", id);
				break;
			case 3:
				vs10xx_inf("id:%d found vs1003 device", id);
				break;
			case 4:
				...
				break;
			case 5:
				vs10xx_inf("id:%d found vs1033 device", id);
				break;
			case 6:
...
unsigned char vs10xx_device_getTxFlag(int id) {
	
	return vs10xx_device[id].txFlag;
}

int vs10xx_device_transmit(int id) {

	int status = 0;
	vs10xx_device[id].txFlag = 0x00;

	
		if (vs10xx_device_getpause(id)) {

			/* on hold, wait for start */
			wait_event_timeout(vs10xx_device[id].wq, vs10xx_device[id].start, msecs_to_jiffies(10));

		}
		else if (vs10xx_queue_isempty(id)) {

			/* no data --> pause */
			if (vs10xx_device_getopen(id)) {

				//vs10xx_nsy("id:%d no data", device->id);
				printk("Device id:%d no data\n", id);
				vs10xx_device_setpause(id);

				if (!vs10xx_device_getfinish(id)) {
		...
		} else if (!vs10xx_io_wtready(id, 10)) {

			/* io not ready to receive */
			printk("Device id:%d not ready\n", id);

		} else {

			mutex_lock(&vs10xx_device[id].lock);
						
			while (vs10xx_io_isready(id) && !vs10xx_queue_isempty(id)) {
				
				/* transmit data */
				struct vs10xx_queue_buf_t *buffer;
				buffer = vs10xx_queue_gethead(id);
				status = vs10xx_io_data_tx(id, buffer->data, buffer->len);
				if (status == 0) {
					vs10xx_queue_dequeue(id);					
				}
				udelay(1687);//(1700);
				
			}
			vs10xx_device[id].txFlag = 0x02;
			mutex_unlock(&vs10xx_device[id].lock);
		}
...
int vs10xx_device_release(int id) {
	...
	printk("buffer length, buffer.len: %d\n", buffer.len);
...
int vs10xx_device_init(int id, struct device *dev) {
	...
	int status = 0;
	unsigned char WRAM_add[2], WRAM_data[2];

	vs10xx_dbg("start:%d", id);
	printk("Inside vs10xx_device_init(). \n");
	...
	/* Write to WRAM to deinitialize MIDI Mode, & set to Mp3 Mode */
	WRAM_add[0] = 0x17, WRAM_add[1] = 0xC0;
	WRAM_data[0] = 0x03, WRAM_data[1] = 0x00;
	status = vs10xx_device_w_WRAM(id, WRAM_add, WRAM_data);
	WRAM_add[0] = 0x19, WRAM_add[1] = 0xC0;
	WRAM_data[0] = 0x00, WRAM_data[1] = 0x00;
	status = vs10xx_device_w_WRAM(id, WRAM_add, WRAM_data);
	#if 0
	if (status == 0) {
